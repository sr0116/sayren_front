...
export function useApiQuery(key, url, { params, options } = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      return await api.get(resolveUrl(url, params));
    },
    ...options,
  });
}

export function useApiMutation(method, url, { invalidateKeys = [], options } = {}) {
  ...
  return useMutation({
    mutationFn: async ({ data, params } = {}) => {
      ...
      switch (method.toUpperCase()) {
        ...
        case "PATCH":
          return await api.patch(targetUrl, data);
        case "DELETE":
          return await api.delete(targetUrl, { data });
        default:
