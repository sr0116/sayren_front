...
export default function ProductCartOrBuy({ productId, price, type }) {
  ...
  // ⭐ 렌탈 상세 페이지에서는 기본 36개월을 선택한 상태로 시작
        ...
        <div>
          ...
          <div className="flex gap-3 mt-2">
            ...
            <Button
                ...
                onClick={() =>
                    router.push(
                        `/order/checkout/${productId}?planId=1&type=PURCHASE`
                    )
                }
            >
              바로 구매
  ...
  const { monthlyFee, deposit, totalPrice } = calcRentalPrice(
      price,
      selectedPlan.month
  );
      ...
      <div>
        ...
        <div className="flex gap-3 mt-2">
          ...
          <Button
              ...
              onClick={() => {
                router.push(
                    `/order/checkout/${productId}?planId=${selectedPlan.planId}&type=RENTAL`
                );
              }}
          >
            바로 구매
